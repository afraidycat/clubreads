        ...
        <div className="grid lg:grid-cols-3 gap-8 overflow-visible">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {completedBooks.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle>Past Reads</CardTitle>
                  <CardDescription>Books your club has completed</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {completedBooks.map(book => (
                      <div key={book.id} className="flex items-center justify-between p-3 rounded-lg bg-[var(--muted)]/50">
                        <div>
                          <p className="font-medium">{book.title}</p>
                          <p className="text-sm text-[var(--muted-foreground)]">by {book.author}</p>
                        </div>
                        {book.theme && (
                          <Badge variant="amber">{book.theme.icon}</Badge>
                        )}
                      </div>
                    ))}
                  </div>
